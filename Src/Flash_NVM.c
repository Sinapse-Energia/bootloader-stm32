...
uint32_t FlashNVM_GetBankSize(FLASH_BANK fl_bank)
{
	...
	if (fl_bank == FLASH_BANK_APPLICATION) {
			return FlashNVM_GetSectorSize(FLASH_BANKA_START_SECTOR, FLASH_BANKA_START_SECTOR + FLASH_BANKA_SECTORS - 1);
	}

	if (fl_bank == FLASH_BANK_COPY) {
			return FlashNVM_GetSectorSize(FLASH_BANKC_START_SECTOR, FLASH_BANKC_START_SECTOR + FLASH_BANKC_SECTORS - 1);
	}

	if (fl_bank == FLASH_BANK_SHARED) {
			return FlashNVM_GetSectorSize(FLASH_BANKD_START_SECTOR, FLASH_BANKD_START_SECTOR + FLASH_BANKD_SECTORS - 1);
	}
...
uint32_t FlashNVM_GetBankStartAddress(FLASH_BANK fl_bank)
{
	...
	if (fl_bank == FLASH_BANK_SHARED) {
		return FLASH_BASE + FlashNVM_GetSectorSize(FLASH_SECTOR_0, FLASH_BANKD_START_SECTOR) - FlashNVM_GetSectorSize(FLASH_BANKD_START_SECTOR, FLASH_BANKD_START_SECTOR);
	}
	return 0;
}
